...
export const careerProjects: CareerProject[] = [
  {
    ...
    challenge: '소득세법·급여계산 지식이 전무한 신입사원 상태에서 급여 로직 설계 난항 → 세법 독학 후 고객사 급여 체계에 맞춰 직접 설계·구현',
    result: '첫 프로젝트 배포 후 이슈 제로 달성, 이후 급여 도메인 전문성 확보',
  ...
  },
  {
    ...
    challenge: '회계 지식 전무 상태에서 전표·예산통제 개발 + 공통팀 돌연 이탈로 내부결제 로직에 문제 발견 → 업무 파트너와 협업으로 회계 구현, 내부결제는 직접 재설계',
    result: 'HR 파트 조기 완료 → 회계·공통 파트까지 추가 지원하여 프로젝트 완수',
  ...
  },
  {
    ...
    challenge: '인력회사 특성상 독자적 업무 방식으로 표준화 어려움 → 고객사 담당자와 주 2회 미팅으로 관리회계 프로세스를 정립하고, 수기 관리를 자동화',
    result: '약 10,000명 사용자 대상 서비스 운영, 급여 프로시저 3분→12초 튜닝',
  ...
  },
  {
    ...
    challenge: 'Map/Layer 경험 없는 상태에서 나라 지원 API 사용 중 데이터 누락 문제 발견 → 긴급 수정. 영상 정사 API가 건바이건 순차 처리로 병목 → 병행처리로 전환',
    result: '2개월 단기 프로젝트 발표까지 이슈 없이 완료, 정사 처리 시간 대폭 단축',
    image: null,
  },
  {
    id: 5,
    ...
    challenge: '조직관리·사업소관리·회계코드관리가 각각 분리되어 중복·불일치 발생 → 권한·조직도와 연동된 통합 프로세스로 재설계 진행중',
    result: '첫 PL 프로젝트로 9명 팀 총괄 중 (진행중)',
